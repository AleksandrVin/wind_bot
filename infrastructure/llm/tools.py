...
class WindSpeedToMSTool(BaseTool):
    ...
    def _run(self, speed: float) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"original_speed": speed, "original_unit": "knots", "converted_speed": ms, "converted_unit": "m/s"}
        except Exception as e:
            return {"error": str(e)}
...
class GetCurrentWeatherInput(BaseModel):
    ...
    location: Optional[str] = Field(
        None, description="Optional location name. If not provided, the default location will be used."
    )
    ...
    def _run(self, location: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "wind": {
                    "speed": {"value_ms": weather_data.wind.speed_ms, "value_knots": weather_data.wind.speed_knots},
                    "gust": {"value_ms": weather_data.wind.gust_ms, "value_knots": weather_data.wind.gust_knots}
                    if weather_data.wind.gust_ms
                    else None,
                },
                "conditions": [{"main": c.main, "description": c.description} for c in weather_data.weather_conditions],
